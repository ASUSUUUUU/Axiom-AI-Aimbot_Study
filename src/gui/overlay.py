...
class PyQtOverlay(QWidget):
    ...
    def draw_fov_corners(self, painter, cx, cy, fov, corner_length=20):
        ...
        在 FOV 框的四個角落繪製 L 形標記線，而非完整的矩形框。
        這種設計可以減少視覺干擾，同時仍能清楚標示 FOV 範圍。
        
        Args:
            painter: QPainter 繪圖物件
            cx, cy: FOV 中心點座標
            fov: FOV 框的邊長（正方形）
            corner_length: 角落標記線的長度，預設 20 像素
        """
        x1 = cx - fov // 2
        y1 = cy - fov // 2
        x2 = cx + fov // 2
        ...
        # 左上角
        painter.drawLine(x1, y1, x1 + corner_length, y1)  # 水平線
        painter.drawLine(x1, y1, x1, y1 + corner_length)  # 垂直線
        
        # 右上角
        painter.drawLine(x2, y1, x2 - corner_length, y1)  # 水平線
        painter.drawLine(x2, y1, x2, y1 + corner_length)  # 垂直線
        
        # 左下角
        painter.drawLine(x1, y2, x1 + corner_length, y2)  # 水平線
        painter.drawLine(x1, y2, x1, y2 - corner_length)  # 垂直線
