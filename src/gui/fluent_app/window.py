...
class AxiomWindow(FluentWindow):
    def __init__(self):
        ...
        # 尊重目前 qfluentwidgets 全域主題（可能已被精靈設置過）
    ...
    def check_for_updates(self):
        ...
        # self.update_checker.up_to_date.connect(lambda: print("已是最新版本")) # Debug用
        # self.update_checker.check_failed.connect(lambda e: print(f"更新檢查失敗: {e}"))
    ...
    def on_update_available(self, new_version, url, body):
        ...
        title = self.langManager.get("update_available_title", "發現新版本")
        content = self.langManager.get("update_available_content", "檢測到新版本 {new_version}，是否前往下載？\n\n當前版本: v{current_version}")
        content = content.replace("{new_version}", new_version).replace("{current_version}", __version__)

        
        # 簡單處理 release notes，截斷太長的部分
        if body:
             # 只取前幾行作為預覽
             preview = "\n".join(body.split('\n')[:5])
             if len(body.split('\n')) > 5:
        ...
        # 設置按鈕文字
        w.yesButton.setText(self.langManager.get("update_yes", "前往下載"))
        w.cancelButton.setText(self.langManager.get("update_no", "稍後再說"))
    ...
    def _applyWindowRoundedCorners(self):
        """設定 Windows 11 視窗圓角 (DWMWA_WINDOW_CORNER_PREFERENCE)
        
        Win11 支援 DWM 圓角選項：
        - DWMWCP_DEFAULT (0): 預設
        - DWMWCP_DONOTROUND (1): 不圓
        - DWMWCP_ROUND (2): 圓角
    ...
    def setConfig(self, config):
        ...
        if saved_dark != self._isDarkTheme:
            if saved_dark:
                setTheme(Theme.DARK)
